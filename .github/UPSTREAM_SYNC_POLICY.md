...
The upstream repository is treated as an external software supplier. Popularity, previous clean reviews, or a clean prior sync do not make future commits trusted automatically.

## Sync path

1. `.github/workflows/upstream-sync.yml` fetches `Panniantong/Agent-Reach:main`.
...
- `agent_reach/cli.py` — installers, subprocess execution, global package installation and skill installation.
- `agent_reach/config.py` — credential persistence, file permissions and secret redaction.
- `agent_reach/cookie_extract.py` — browser-session and cookie access boundaries.
- `agent_reach/backends/**` and `agent_reach/channels/**` — external commands, browser control, authentication and network access.
- `pyproject.toml` and `constraints.txt` — new or changed dependencies and version pins.
- Any new executable scripts, downloaded binaries, MCP servers, browser extensions, or remote install sources.
...
Do not configure the upstream-sync PR for automatic merging.
...
If a security incident requires bypassing normal review, document the reason and the exact commit SHA in the PR or incident record, then restore the normal protection immediately after the emergency change.
